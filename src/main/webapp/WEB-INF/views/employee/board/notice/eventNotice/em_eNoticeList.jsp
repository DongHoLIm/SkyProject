...
<body>
	<div id="wrapper">
		<div id="main">
			...
			<form action="em_searcheNotice.bo" method="get">
			...
			<c:if test="${pi.searchflag == true}">
				...
				<div class="pagingArea">
				  <ul class="pagination" style="justify-content: center; cursor: pointer;">
					...
					<c:if test="${pi.currentPage > 1}">
						<c:url var="blistBack" value="em_searcheNotice.bo">
					...
					<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
						...
						<c:if test="${p ne pi.currentPage}">
							<c:url var="blistCheck" value="em_searcheNotice.bo">
								<c:param name="searchCondition" value="${searchCondition}"/>													
								<c:param name="searchValue" value="${searchValue}"/>
					...
					<c:if test="${pi.currentPage < pi.maxPage }">
						<c:url var="blistEnd" value="em_searcheNotice.bo">
							<c:param name="searchCondition" value="${searchCondition}"/>														
							<c:param name="searchValue" value="${searchValue}"/>
							<c:param name="currentPage" value="${pi.currentPage + 1}"/>
							<c:param name="searchflag" value="${pi.searchflag}"/>
			...
			<div class="pagingArea">
			  <ul class="pagination" style="justify-content: center; cursor: pointer;">
				...
				<c:if test="${pi.currentPage > 1}">
					<c:url var="blistBack" value="em_eNoticeList.bo">
						<c:param name="currentPage" value="${pi.currentPage - 1}"/>
					</c:url>
					<li class="page-item"><a class="page-link" href="${blistBack}">이전</a></li>	
				</c:if>
				<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
					...
					<c:if test="${p ne pi.currentPage}">
						<c:url var="blistCheck" value="em_eNoticeList.bo">
							<c:param name="currentPage" value="${p}"/>
						</c:url>
						 <li class="page-item"><a class="page-link" href="${blistCheck}">${p}</a></li>
					</c:if>
				...
				<c:if test="${pi.currentPage < pi.maxPage }">
					<c:url var="blistEnd" value="em_eNoticeList.bo">
						<c:param name="currentPage" value="${pi.currentPage + 1}"/>
					</c:url>
			    	<li class="page-item"><a class="page-link" href="${blistEnd}">다음</a></li>				
				</c:if>
	...
	<script>	
		...
		function inserteNotice(){
			var memberId = $("input[name=memberId]").val();			
			
			location.href = "em_showInserteNotice.bo?memberId=" + memberId;
		}		
