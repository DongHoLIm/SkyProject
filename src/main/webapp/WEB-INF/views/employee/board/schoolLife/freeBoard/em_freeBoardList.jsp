...
<body>
	<div id="wrapper">
		<div id="main">
			...
			<form action="em_freeBoardsearch.bo" method="get">
			...
			<div id="writeArea">
				<div align="right" style="margin-bottom: 10px;">
					...
					<button id="writeBtn" onclick="insertfreeBoard()">글쓰기</button>
				</div>
			</div>
			...
			<c:if test="${pi.searchflag == true}">
				...
				<div class="pagingArea">
				  <ul class="pagination" style="justify-content: center; cursor: pointer;">
					...
					<c:if test="${pi.currentPage > 1}">
						<c:url var="blistBack" value="em_freeBoardsearch.bo">
					...
					<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
						...
						<c:if test="${p ne pi.currentPage}">
							<c:url var="blistCheck" value="em_freeBoardsearch.bo">
								<c:param name="searchCondition" value="${searchCondition}"/>													
								<c:param name="searchValue" value="${searchValue}"/>
					...
					<c:if test="${pi.currentPage < pi.maxPage }">
						<c:url var="blistEnd" value="em_freeBoardsearch.bo">
							<c:param name="searchCondition" value="${searchCondition}"/>														
							<c:param name="searchValue" value="${searchValue}"/>
							<c:param name="currentPage" value="${pi.currentPage + 1}"/>
							<c:param name="searchflag" value="${pi.searchflag}"/>
			...
			<c:if test="${pi.searchflag == false}">
				...
				<div class="pagingArea">
				  <ul class="pagination" style="justify-content: center; cursor: pointer;">
					...
					<c:if test="${pi.currentPage > 1}">
						<c:url var="blistBack" value="em_freeBoardList.bo">
							<c:param name="searchCondition" value="${searchCondition}"/>																					
							<c:param name="searchValue" value="${searchValue}"/>
							<c:param name="currentPage" value="${pi.currentPage - 1}"/>
							<c:param name="searchflag" value="${pi.searchflag}"/>
					...
					<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
						...
						<c:if test="${p ne pi.currentPage}">
							<c:url var="blistCheck" value="em_freeBoardList.bo">
								<c:param name="searchCondition" value="${searchCondition}"/>													
								<c:param name="searchValue" value="${searchValue}"/>
								<c:param name="currentPage" value="${p}"/>	
								<c:param name="searchflag" value="${pi.searchflag}"/>
					...
					<c:if test="${pi.currentPage < pi.maxPage }">
						<c:url var="blistEnd" value="em_freeBoardList.bo">
							<c:param name="searchCondition" value="${searchCondition}"/>														
							<c:param name="searchValue" value="${searchValue}"/>
							<c:param name="currentPage" value="${pi.currentPage + 1}"/>
							<c:param name="searchflag" value="${pi.searchflag}"/>
	...
	<script>
		$(function(){
			 $("#boardArea").find(".freeBoardTitle").click(function(){
				...
				location.href = "em_freeBoardDetail.bo?boardNo=" + boardNo;				
			 });
		});
		...
		function insertfreeBoard(){
			...
			location.href = "em_showInsertfreeBoard.bo?memberId=" + memberId;
		}	
	</script>	
</body>
</html>