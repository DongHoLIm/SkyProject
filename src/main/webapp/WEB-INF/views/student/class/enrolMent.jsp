...
<script language="JavaScript" src="/html/js/jquery.min.js"></script>
<script language="JavaScript" src="/html/js/jquery-1.8.3.js"></script>
<script language="javascript" charset="euc-kr"
	src="/html/js/gwamok/top_20192.js?fake=Wed Sep 04 10:11:21 KST 2019"></script>
<script type="text/javascript" src="/html/js/netfunnel.js"></script>
<!--  netfunnel 기본파일 -->

  
<script language="javascript">
	// ------------------------------------------ 딜레이 ---------------------------------------------- //
	function btn_waitingEnable() {
		$("#waiting_btn").hide();
		$("#search_btn").show();
	}

	function btn_waitingCall() {

		$("#search_btn").hide();
		$("#waiting_btn").show();
		setTimeout("btn_waitingEnable()", 2000); // 2초가 지난후에 다시 버튼을
	}
	// ------------------------------------------ 딜레이 ---------------------------------------------- //	

	function setMajor(v_mj_cd) {

		var nOption;
		var li_sel = 0;
		var li_add = 0;
		document.form.allMajor.length = 0;
		for (var i = 0; i < allMajor.length; i++) {
			nOption = document.form.allMajor.length;

			document.form.allMajor.options[nOption] = new Option(
					allMajor[i].description, allMajor[i].nvalue);
			if (v_mj_cd == allMajor[i].nvalue)
				li_sel = li_add;
			li_add++;
		}

		if (document.form.allMajor.length > 0) {
			document.form.allMajor.options[li_sel].selected = true;
			//document.form.allMajor.options[0].selected=true; 
		}
	...
	function setIsu() {
		var nOption;
		document.form.allIsu.length = 0;
		var li_sel = 0;
		var v_major_cd = $("#allMajor").val();

		document.form.allIsu.options[0] = new Option("* 전체 *", "ALL");

		for (var i = 0; i < allIsu.length; i++) {
			nOption = document.form.allIsu.length;
			if (v_major_cd == allIsu[i].major_cd) {
				document.form.allIsu.options[nOption] = new Option(
						allIsu[i].description, allIsu[i].nvalue);
			}
		}

		if (document.form.allIsu.length > 0) {
			document.form.allIsu.options[li_sel].selected = true;
		} else {
			document.form.allIsu.options[0] = new Option("자료없음", "-1");
		}

		// 학년 초기화
		document.form.allHakyun.options[0].selected = true;
	...
	function setLecture() {
		var nOption;
		document.form.allLecture.length = 0;
		var li_sel = 0;
		var v_major_cd = $("#allMajor").val();
		var v_isu_cd = $("#allIsu").val();
		var v_hakyun = $("#allHakyun").val();

		// 교양, 교직학과 제외
		if (v_major_cd == '400000' || v_major_cd == '400210') {

		} else {
			document.form.allLecture.options[0] = new Option("* 전체 *", "ALL");
		}

		for (var i = 0; i < allLecture.length; i++) {
			nOption = document.form.allLecture.length;
			if (v_major_cd == allLecture[i].major_cd
					&& (v_isu_cd == 'ALL' || v_isu_cd == allLecture[i].isu_cd)
					&& (v_hakyun == 'ALL' || allLecture[i].hakyun
							.indexOf(v_hakyun) >= 0)) {
				document.form.allLecture.options[nOption] = new Option(
						allLecture[i].description + " [" + allLecture[i].nvalue
								+ "]", allLecture[i].nvalue);
			}
		}

		if (document.form.allLecture.length > 0) {
			document.form.allLecture.options[li_sel].selected = true;
		} else {
			document.form.allLecture.options[0] = new Option("개설강좌 없음", "0");
		}
	}
	...
	function search() {
		...
		/* 넷퍼넬 막음 2015.07.06 
		top.NetFunnel_goFunc({},function(ev,ret){   
					onSuccess_search();
		    	}  
		    );		
		 */
	...
	function onSuccess_search() {
		var category = document.form.category.value;

		//Netfunnel complete call
		NetFunnel_goComplete();

		// 1.학과,전공으로 조회
		if (category == "0") {
			document.form.sbjt_cd.value = $("#allLecture").val();
			url = "/stud/core?attribute=lect_list";
			// 2.과목명으로 조회	
		} else if (category == "1") {
			if ($("#gwamok_cd").val() == null || $("#gwamok_cd").val() == "") {
				alert("과목명으로 강좌조회를 먼저하세요!");
				document.form.gwamok_nm.focus();
				return;
			}
			document.form.sbjt_cd.value = $("#gwamok_cd").val();
			url = "/stud/core?attribute=lect_list";
			// 3.교수명으로 조회	
		} else if (category == "2") {
			document.form.sbjt_cd.value = $("#profLecture").val();
			url = "/stud/core?attribute=lect_list";
		}

		document.form.action = url;
		document.form.method = "POST";
		document.form.target = "frm2";
	...
	//과목명으로 과목찾기
	function findAllLecture(gwamok_nm) {
		var selValue;
		var nOption;

		selValue = gwamok_nm;
		document.form.gwamok_cd.length = 0;

		for (var i = 0; i < allSearch.length; i++) {
			if (allSearch[i].description.toUpperCase().indexOf(
					selValue.toUpperCase()) >= 0
					|| allSearch[i].nvalue.toUpperCase() == selValue
							.toUpperCase()) {
				nOption = document.form.gwamok_cd.length;
				document.form.gwamok_cd.options[nOption] = new Option(
						allSearch[i].description + " [" + allSearch[i].nvalue
								+ "]", allSearch[i].nvalue);
			}
		}
	}

	function search_gwamok_nm() {

		// ie8에서는 trim()이 안먹어서 추가함.
		...
		var gwamok_nm = document.form.gwamok_nm.value;

		/*	
		if(gwamok_nm.trim().length < 2 && gwamok_nm.trim().length > 0 ) {
			alert("2글자 이상 입력하세요!");
			return true;	
		}
		 */

		if (gwamok_nm.trim().length > 0) {
			findAllLecture(gwamok_nm);
			// 과목명이 없을 경우
		} else {
			document.form.gwamok_nm.value = "과목명(코드)을 입력하세요.";
			return;
		}

	}

	function click_gwamok_nm() {
		if (document.form.gwamok_nm.value == "과목명(코드)을 입력하세요.") {
			document.form.gwamok_nm.value = "";
		}
	}

	// ------------------------------------------ 교수 개설과목 조회 ---------------------------------------------- //
	//교수명으로 교수찾기
	function findAllProf(prof_nm) {
		var selValue;
		var nOption;

		selValue = prof_nm;
		document.form.allProf.length = 0;

		for (var i = 0; i < allProf.length; i++) {
			if (allProf[i].description.toUpperCase().indexOf(
					selValue.toUpperCase()) >= 0) {
				nOption = document.form.allProf.length;
				document.form.allProf.options[nOption] = new Option(
						allProf[i].description, allProf[i].nvalue);
			}
		}

		if (document.form.allProf.length > 0) {
			document.form.allProf.options[0].selected = true;
		} else {
			document.form.allProf.options[0] = new Option("자료없음", "0");
		}

		setProfLecture();
	}

	function search_prof_nm() {
		...
		var prof_nm = document.form.prof_nm.value;

		if (prof_nm.trim().length > 0) {
			findAllProf(prof_nm);
			// 교수명이 없을 경우
		} else {
			document.form.prof_nm.value = "교수명을 입력하세요.";
			return;
		}

	}

	function click_prof_nm() {
		if (document.form.prof_nm.value == "교수명을 입력하세요.") {
			document.form.prof_nm.value = "";
		}
	}

	function setProfLecture() {
		var nOption;
		document.form.profLecture.length = 0;
		var li_sel = 0;
		var v_prof_cd = $("#allProf").val();

		for (var i = 0; i < allLectureProf.length; i++) {
			nOption = document.form.profLecture.length;

			if (v_prof_cd == allLectureProf[i].prof1
					|| v_prof_cd == allLectureProf[i].prof2
					|| v_prof_cd == allLectureProf[i].prof3
					|| v_prof_cd == allLectureProf[i].prof4) {
				document.form.profLecture.options[nOption] = new Option(
						allLectureProf[i].description + " ["
								+ allLectureProf[i].nvalue + "]",
						allLectureProf[i].nvalue);
			}

		}

		if (document.form.profLecture.length > 0) {
			document.form.profLecture.options[li_sel].selected = true;
		} else {
			document.form.profLecture.options[0] = new Option("개설강좌 없음", "0");
		}
	}
	// ------------------------------------------ 교수 개설과목 조회 ---------------------------------------------- //
...
<body bgcolor="#F3F3F3" marginwidth="10" marginheight="14">
<table class="tableList3" width="100%">
    ...
    <tbody>
    	...
    	<td>
				<span>
						&nbsp;&nbsp;&nbsp;<b>카테고리</b> : 
						<select name="category" id="category" onchange="change_cate()"
						style="background-color:#FFD9EC;">
							<option value="0" selected="">학과 조회</option>
				...
				<span id="choice0" name="choice0" style="display: inline;">						
				    <span>
					    <strong>학과</strong> : <select name="allMajor" id="allMajor"
							class="Layer1" onchange="javascript:setIsu();"><option
									value="400000"># 교양과정  [400000]</option>
								...
								<option value="410910">휴먼서비스학부  [410910]</option></select> &nbsp;&nbsp;&nbsp;
					    <strong>이수구분</strong> : <select name="allIsu" id="allIsu"
							class="Layer1" onchange="javascript:setLecture();"><option
									value="ALL">* 전체 *</option>
								<option value="1">교필</option>
								<option value="2">교선</option>
								<option value="P">평생</option></select> &nbsp;&nbsp;&nbsp;
					    <strong>학년</strong> : <select name="allHakyun" id="allHakyun"
							class="Layer1" onchange="javascript:setLecture();">			
					    ...
					    <strong>과목</strong> : <select name="allLecture" id="allLecture"
							class="Layer1" style="background-color:#FFD9EC;"><option
									value="KY508">1인미디어만들기 [KY508]</option>
								...
								<option value="KY607">현장으로이해하는아시아 [KY607]</option></select>&nbsp;&nbsp;&nbsp;
			  		</span>	  		
			  	</span>
			  	<!-- 과목조회 -->
			  	<span id="choice1" name="choice1" style="display: none;">
			  		<span>
				    	<span class="ui-widget">
						  ...
						  <input type="text" name="gwamok_nm" id="gwamok_nm" size="20"
								value="과목명(코드)을 입력하세요."
								onkeydown="if(event.keyCode == 13) {search_gwamok_nm(); return false;}"
								onblur="search_gwamok_nm();" onfocus="click_gwamok_nm();">
						  	과목 :
						   <select name="gwamok_cd" id="gwamok_cd" class="Layer1"
								style="width:200;background-color:#FFD9EC;"></select>	&nbsp;&nbsp;&nbsp; 
						</span>
					</span>
		  		</span>
		  		<!-- 교수조회 -->
				<span id="choice2" name="choice2" style="display:none">				
				    <span>
				    	<span class="ui-widget">
						  <label for="prof_nm">조회 : </label>
						  <input type="text" name="prof_nm" id="prof_nm" size="20"
								value="교수명을 입력하세요."
								onkeydown="if(event.keyCode == 13) {search_prof_nm(); return false;}"
								onblur="search_prof_nm();" onfocus="click_prof_nm();">
						    교수 : <select name="allProf" id="allProf" class="Layer1"
								style="width:150;" onchange="javascript:setProfLecture();"></select> &nbsp;&nbsp;&nbsp;
						  	과목 :
						   <select name="profLecture" id="profLecture" class="Layer1"
								style="width:200;background-color:#FFD9EC;"></select>	&nbsp;&nbsp;&nbsp; 
						</span>
					</span> 		
			  	</span>
			  	<span>
			  		<span class="btn red on" onclick="javascript:search();"
						name="search_btn" id="search_btn"> 조 회 </span>	
			  		<span name="waiting_btn" id="waiting_btn" style="display:none">&nbsp;&nbsp;<img
							src="/html/images/loading_on.gif" style="margin: 0px;"></span>
			  	</span>
...
</html><table class="tableList2" style="width: 100%">
		<tbody>
			<tr>
				<th width="35">No</th>
				<th width="45">학년</th>
